        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT * from  commoffers");
...
<script>
	$(document).ready(function(){
		$('.delete_data').click(function(){
			_conf("Are you sure to delete this offer permanently?","delete_commoffers",[$(this).attr('data-id')])
		})
		$('.table').dataTable({
			columnDefs: [
	...
	function delete_commoffers($ID){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=delete_commoffers",
			method:"POST",
			data:{ID: $ID},
			dataType:"json",
			error:err=>{
