        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT * from  sellsoffers");
...
<script>
	$(document).ready(function(){
		$('.delete_data').click(function(){
			_conf("Are you sure to delete this offer permanently?","delete_offer",[$(this).attr('data-id')])
		})
		$('.table').dataTable({
			columnDefs: [
	...
	function delete_offer($ID){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=delete_offer",
			method:"POST",
			data:{ID: $ID},
			dataType:"json",
			error:err=>{
