        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT t.*, concat(t.firstname,' ',COALESCE(concat(t.middlename,' '), ''), t.lastname) as customer, v.mv_number, v.plate_number, v.price, m.model, m.engine_type, m.transmission_type, b.name as `brand`, ct.name as `car_type` , Year from transaction_list t inner join `vehicle_list` v on t.vehicle_id = v.id inner join model_list m on v.model_id = m.id inner join brand_list b on m.brand_id = b.id inner join car_type_list ct on m.car_type_id = ct.id where v.status = 1 and v.delete_flag = 0 order by abs(unix_timestamp(v.date_created)) asc ");
