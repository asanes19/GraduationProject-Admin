...
Class Master extends DBConnection {
	...
	function delete_offer(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `sellsoffers` WHERE `ID` = '{$ID}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Offer successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function delete_sugg(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `suggestions` WHERE `id` = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," sugg successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function delete_clints(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `clients` WHERE `id` = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," clint successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function delete_commoffers(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `commoffers` WHERE `ID` = '{$ID}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Offer successfully deleted.");
		}else{
	...
	function delete_pic(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `images` WHERE `id` = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Pic successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function delete_requstedcars(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `requstcars` WHERE `ID` = '{$ID}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Offer successfully deleted.");
		}else{
	...
	function save_Biddingvehicle(){
		...
		if(empty($id)){
			$sql = "INSERT INTO `biddingcars` set {$data} ";
		}else{
			$sql = "UPDATE `biddingcars` set {$data} where id = '{$id}' ";
		}
			$save = $this->conn->query($sql);
		if($save){
			$vid = !empty($id) ? $id : $this->conn->insert_id;
	...
	function save_news(){
		...
		$check = $this->conn->query("SELECT * FROM `news` where `Pic` = '{$Pic}' and Title = '{$Title}' and Descriptionn = '{$Descriptionn}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
...
switch ($action) {
	...
	break;
	case 'delete_sugg':
		echo $Master->delete_sugg();
	break;
	case 'delete_model':
		echo $Master->delete_model();
	break;
	case 'delete_offer':
		echo $Master->delete_offer();
	break;
	case 'delete_clints':
		echo $Master->delete_clints();
	break;
	case 'delete_commoffers':
		echo $Master->delete_commoffers();
	break;
	case 'delete_requstedcars':
		echo $Master->delete_requstedcars();
	break;
	case 'delete_pic':
		echo $Master->delete_pic();
	break;
	case 'save_vehicle':
		echo $Master->save_vehicle();
	break;
	case 'save_Biddingvehicle':
		echo $Master->save_vehicle();
	break;
	case 'delete_vehicle':
